...
def get_ds(config):
    ...
    ds_raw = load_dataset('ai4bharat/samanantar', f'{config["lang_tgt"]}', split='train[:50000]')

    # Build tokenizers
    tokenizer_src = get_or_build_tokenizer(config, ds_raw, "src")
    tokenizer_tgt = get_or_build_tokenizer(config, ds_raw, "tgt")
    ...
    for item in ds_raw:
        ...
        tgt_ids = tokenizer_src.encode(item["tgt"]).ids
